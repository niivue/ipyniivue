...
const SKIPPED_NOTEBOOKS = new Set<string>([
  "example_sideview.html",
]);
...
test.describe("ipyniivue static HTML (e2e) — generated from examples", () => {
  ...
  console.log(`[prepare-e2e] Found ${htmlFiles.length} html files under ${HTML_DIR}`);
  ...
  for (const htmlFile of htmlFiles) {
    ...
    test(title, async ({ page }) => {
	  const shouldSkip = SKIPPED_NOTEBOOKS.has(rel) || SKIPPED_NOTEBOOKS.has(base);

	  // debug summary visible at collection time
	  console.log(`[prepare-e2e] define: ${rel}  shouldSkip=${shouldSkip}`);
	  if(shouldSkip)
		test.skip(shouldSkip, `${testName} in exclude list`)
