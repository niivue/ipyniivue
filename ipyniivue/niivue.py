...
class Niivue(_CanvasBase):
    ...
    def set_volume(self, volume, to_index = 0):
        ...
        to_index: int
			the index to move the volume to. The default is the background (index 0)
        '''
    ...
    def set_slice_type(self, slice_type):
        ...
        -----------
        slice_type: int
			the slice type. Valid values are nv.slice_type.axial, nv.slice_type.coronal, nv.slice_type.sagittal, nv.slice_type.multiplanar, and nv.slice_type.render
    ...
    def add_volume(self, file):
        ...
        if (file.startswith('http://') or file.startswith('https://')):
            self._send_custom(['addVolumeFromUrl', [{'url': file}]])
        else:
    ...
    def getDescriptives(self, layer, masks):
        return self.run_custom_code(f'this.nv.getDescriptives({layer}, {masks})')

    def getFrame4D(self, ID):
        return self.run_custom_code(f'this.nv.getFrame4D({ID})')

    def getMediaByUrl(self, url):
        return self.run_custom_code(f'this.nv.getMediaByUrl({url})')

    def getOverlayIndexByID(self, ID):
        return self.run_custom_code(f'this.nv.getOverlayIndexByID({ID})')

    def getRadiologicalConvention(self):
        return self.run_custom_code(f'this.nv.getRadiologicalConvention()')

    def getVolumeIndexByID(self, ID):
        return self.run_custom_code(f'this.nv.getVolumeIndexByID({ID})')

    def isMeshExt(self, url):
