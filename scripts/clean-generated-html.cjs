...
const { readdirSync, statSync, unlinkSync, rmdirSync, existsSync } = require('fs');
...
(function main() {
  ...
  if (!existsSync(outAbs)) {
    console.log(`[clean-generated-html] nothing to do — OUT_DIR does not exist: ${outAbs}`);
    process.exit(0);
  }

  console.log(`[clean-generated-html] OUT_DIR = ${outAbs}`);
  if (DRY_RUN) console.log('[clean-generated-html] running in dry-run mode (no files will be deleted)');
  ...
  if (toDelete.length === 0) {
    console.log('[clean-generated-html] no generated html/executed notebooks/thumbnails found to delete.');
    process.exit(0);
  }

  console.log(`[clean-generated-html] found ${toDelete.length} items to remove:`);
  ...
  for (const f of toDelete) {
    try {
      ...
      console.error(`[clean-generated-html] failed to delete ${f}:`, err && err.message ? err.message : err);
    }
  }
