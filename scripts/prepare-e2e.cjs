...
const RETRY_ALLOW_ERRORS = process.env.RETRY_ALLOW_ERRORS !== "0";
...
function runExecNotebook(notebookPath, outDir, options = {}) {
	...
	console.log(
		`[prepare-e2e] running: ${nodeCmd} ${args.map((a) => (a.includes(" ") ? JSON.stringify(a) : a)).join(" ")}`,
	);
	const sp = spawnSync(nodeCmd, args, {
		env: spawnEnv,
		encoding: "utf8",
...
(async function main() {
	try {
		...
		if (notebooks.length === 0) {
			console.error(
				`[prepare-e2e] No notebooks found in "${NOTEBOOKS_DIR}". Nothing to prepare.`,
			);
			process.exit(1);
		}
		...
		for (const nbRel of notebooks) {
			...
			try {
				if (existsSync(expectedHtmlPath)) {
					...
					if (s.size > 0) {
						console.log(
							`[e2e] ✔ skipping ${nbName} — existing HTML found at ${expectedHtmlPath}`,
						);
						summary.succeeded.push({ notebook: nbRel, html: expectedHtmlPath });
						continue; // next notebook
					}
					console.log(
						`[e2e] found ${expectedHtmlPath} but file is empty; will re-generate`,
					);
				}
			} catch (err) {
				console.warn(
					`[e2e] warning checking existing HTML for ${nbName}:`,
					err?.message ? err.message : String(err),
				);
			}

			// First attempt: normal
			const res = runExecNotebook(nbRel, OUT_DIR, {
				python: PYTHON,
				timeoutMs: TIMEOUT,
			});
			...
			if (res.status === 0) {
				...
				try {
					...
					console.warn(
						`[e2e] Warning: ${nbName} exited 0 but output could not be parsed as JSON. stdout snippet:`,
					);
				...
				console.warn(
					`[e2e] ✗ first pass failed for ${nbName} (status=${res.status})`,
				);
				if (res.stderr) {
					console.warn(
						`[e2e] stderr (first pass):\n${res.stderr.split("\n").slice(0, 200).join("\n")}`,
					);
				}
			}
			...
			if (RETRY_ALLOW_ERRORS) {
				...
				if (res2.status === 0) {
					try {
						...
						const parsed = JSON.parse(lastLine);
						console.log(
							`[e2e] ✔ ${nbName} (with ALLOW_ERRORS) -> HTML: ${parsed.htmlPath}`,
						);
						summary.succeeded.push({ notebook: nbRel, html: parsed.htmlPath });
						continue;
					} catch (err) {
						console.warn(
							`[e2e] Warning: retry for ${nbName} exited 0 but JSON parse failed. stdout snippet:`,
						);
						console.warn(res2.stdout.slice(0, 2000));
					}
				} else {
					console.warn(
						`[e2e] ✗ retry failed for ${nbName} (status=${res2.status})`,
					);
					if (res2.stderr) {
						console.warn(
							`[e2e] stderr (retry):\n${res2.stderr.split("\n").slice(0, 200).join("\n")}`,
				...
				console.log(
					"[e2e] RETRY_ALLOW_ERRORS=0; skipping retry with ALLOW_ERRORS",
				);
			}
		...
		for (const s of summary.succeeded) {
			console.log(`  - ${s.notebook} -> ${s.html}`);
		}
		console.log(`Failed: ${summary.failed.length}`);
		for (const f of summary.failed) {
			console.log(`  - ${f.notebook}`);
		}
