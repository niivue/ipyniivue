...
(async function main() {
	...
	if (!nbAbs.startsWith(`${repoRoot}/`))
		exitErr("Notebook must be inside the repo");
	...
	function run(cmd, args, opts = {}) {
		console.log(
			`[exec-notebook] ${cmd} ${args.map((a) => (a.includes(" ") ? JSON.stringify(a) : a)).join(" ")}`,
		);
		const sp = spawnSync(cmd, args, {
			stdio: "inherit",
			timeout,
			env: { ...process.env },
			...opts,
	...
	try {
		...
		const allowErrorsFlag =
			process.env.ALLOW_ERRORS === "1"
		...
		const executeArgs = [
			...
			nbAbs,
			"--output",
			executedBasename, // basename WITHOUT extension
			"--output-dir",
			outDirAbs,
		...
		if (!existsSync(executedPath)) {
			console.error(
				`[exec-notebook] ERROR: expected executed notebook not found at ${executedPath}`,
			);
			console.error(
				"[exec-notebook] Contents of output dir:",
				safeListDir(outDirAbs).join("\n  - "),
			);
		...
		if (!existsSync(htmlPath)) {
			console.error(
				`[exec-notebook] ERROR: expected HTML not found at ${htmlPath}`,
			);
			console.error(
				"[exec-notebook] Contents of output dir:",
				safeListDir(outDirAbs).join("\n  - "),
			...
			if (existsSync(executedPath)) {
				try {
					...
					console.error(
						"[exec-notebook] executed notebook metadata snippet:",
						JSON.stringify(
							j.metadata?.kernelspec || j.metadata?.language_info || {},
							null,
							2,
						),
					);
					// show whether any cell outputs contain errors
					const cellsWithErrors = (j.cells || []).filter((c) =>
						(c.outputs || []).some((o) => o.ename || o.output_type === "error"),
					);
					console.error(
						`[exec-notebook] executed notebook cells with errors: ${cellsWithErrors.length}`,
					);
				} catch (pe) {
					console.error(
						"[exec-notebook] failed to parse executed notebook for diagnostics:",
						pe.message,
					);
				}
			}
		...
		exitErr(
			`Error during conversion: ${err?.message ? err.message : String(err)}`,
		);
	}
})();
