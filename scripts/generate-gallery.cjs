...
// Node script to create thumbnails from notebooks and produce gallery.html
//
// Usage:
...
const NOTEBOOKS_DIR = process.env.NOTEBOOKS_DIR || path.resolve(process.cwd(), 'notebooks');
...
function generateExecutedHtml(nbPath, htmlOutPath) {
  ...
  const cmd = `${PYTHON} -m nbconvert --to html --execute "${nbPath}" --output "${path.basename(htmlOutPath)}" --output-dir "${path.dirname(htmlOutPath)}" --ExecutePreprocessor.timeout=120`;
...
async function processNotebook(nbFilename) {
  ...
  for (const cell of nbJson.cells || []) {
    ...
    for (const out of cell.outputs) {
      ...
      } else if (out.text && typeof out.text === 'string' && out.text.startsWith('data:image/png;base64,')) {
  ...
  if (lastImageBase64) {
    ...
    console.log(`[thumb] embedded image/png extracted -> ${path.relative(process.cwd(), thumbPath)}`);
    return { name: nbFilename, thumbnail: path.relative(OUT_DIR, thumbPath), ok: true };
  }
  ...
  if (!htmlPath) {
    ...
    console.log('[info] no pre-existing HTML found; attempting nbconvert to generate executed HTML (slow)');
    ...
    try {
      ...
      return { name: nbFilename, ok: false, reason: 'no image and nbconvert failed' };
    }
  }
...
(async () => {
  ...
  for (const nb of notebooks) {
    try {
      // produce sequentially to avoid launching many browsers at once; small repo-friendly
      // If you want parallel, hook up a concurrency queue and be careful with nbconvert.
  ...
  const items = results.filter(r => r.ok && r.thumbnail).map(r => {
    const nbRel = path.relative(OUT_DIR, path.join(NOTEBOOKS_DIR, r.name)).split(path.sep).join('/');
    const thumbRel = r.thumbnail.split(path.sep).join('/');
    return { name: r.name, nbRel, thumbRel };
  });
...
`;
  ...
  console.log(`[summary] ${items.length} thumbnails created, ${results.length - items.length} notebooks skipped or failed.`);
